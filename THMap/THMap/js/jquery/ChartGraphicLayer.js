dojo.declare("ChartGraphicLayer", esri.layers.GraphicsLayer, {
    ...
    _draw: function (graphic, redrawFlag, zoomFlag) {
        ...
        if (graphic instanceof ChartGraphics)//addGraphic方法传入的参数判断是否为chartGraphics类
        {
    ...
    _drawChart: function (graphic, redrawFlag, zoomFlag) {
        ...
        if (_bindGraphic.visible && _bindGraphic._extent) {
            ...
            var _gExtent = _bindGraphic._extent;
            //var _gScrCenter = map.toScreen(_gExtent.getCenter());
            //  var _gScrCenter = map.map.toScreen(_bindGraphic.geometry);

            var _gScrCenter = allMap.toScreen(_bindGraphic.geometry);
            //1.根据Geometry设置图表大小
            ...
            //2.固定大小的图表
            //var _chartSize = 50;
            var _chartSize = graphic.size;//将传入的chartGraphics类的size属性赋值为图表大小 
            //3.随着缩放的设置图表大小
            ...
            if (!graphic.parentDiv || zoomFlag) {
                ...
                dojo.style(_chartDiv, {
                    "left": (_gScrCenter.x - _chartSize / 2) + "px",
                ...
                var _sumInfo = dojo.doc.createElement("label");
                _sumInfo.innerText = graphic.sum;
                _chartDiv.appendChild(_sumInfo);
                dojo.style(_sumInfo, {
                    "left": ($(_chartDiv).width() - $(_sumInfo).width()) / 2 + "px",
                    "top": "-5px",
                    "position": "absolute",
                    "color": "red"
                });
